...
function FslWarpPet_help
{
    echo
    echo "FslWarpPet.bash [OPTIONS] Subject1 Subject2 ... SubjectN"
    echo
    echo "USAGE:"
    echo
    echo "The purpose of this script is to warp pet images into mni space using FSL."
    echo "FslAnat.bash needs to run before this batch. The following steps are performed in this order:"
    echo " -linearly coregsiter the pet scans to the anatomical image"
    echo "  The linear coregistration uses the bbr cost function which is great for coregistering"
    echo "  T2 epi functional images to the anatomical (not sure about PET). This is done with flirt"
    echo " -warp the pet images into mni space using applywarp"
    echo
    echo "OPTIONS:"
    ...
    echo "  -c CoregImage       path to image used for coregistration between anatomical and PET images."
    echo "                      This is searched directly below the subject directory."
    echo "                      EXAMPLE: PET/run_01/FileName.nii"
    echo "                      REQUIRED"
...
while (( $# > 0 ))
do
    while getopts I:M:U:a:c:f:g:n:r:v:hP: opt
    do
        case "$opt" in
            h)
                FslWarpPet_help
...
# check if CoregImage was specified
if [ -z "$CoregImage" ]
then
    echo
    echo "CoregImage is empty. THIS FLAG IS REQUIRED."
    echo "Read the help for its purpose."
    echo " * * * A B O R T I N G * * *"
    exit
fi
...
do
    ...
    # check for CoregImage
    tmp=(`ls -d ${MasterDir}/${iSubject}/${CoregImage}`)
    if [ ${#tmp[@]} -gt 1 ]
    then
        echo
        echo "CoregImage points to more than one image."
        echo "Are you using wildcards? Only one image is allowed."
        echo " * * * A B O R T I N G * * *"
        echo
        exit
    fi
    ...
    then
        echo
        echo "CoregImage does not exist."
        echo "Double check your arguments."
        echo " * * * A B O R T I N G * * *"
        echo
        exit
    fi 
    ...
    echo "Subject : ${iSubject} found ${#RunDirec[@]} run(s)"
done
...
do
    ...
    FullCoregImage=`ls -d ${MasterDir}/${iSubject}/${CoregImage}`

    echo "fast_wmseg"
    fslmaths ${SubjAnatDir}/T1_fast_pve_2 -thr 0.5 -bin ${SubjAnatDir}/T1_fast_wmseg

    echo "flirt init"
    flirt -ref ${SubjAnatDir}/T1_biascorr_brain -in ${FullCoregImage} -dof 6 -omat ${SubjAnatDir}/${MatPrefix}2anat_init.mat
